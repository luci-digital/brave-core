...
import { brandedWallpaperLogoClicked, triggerSponsoredRichMediaEvent } from '../../api/wallpaper'
import {
...
import { SponsoredRichMediaBackgroundInfo, SponsoredRichMediaBackground } from './sponsored_rich_media_background'
...
function GetSponsoredRichMediaBackground (props: Props): SponsoredRichMediaBackgroundInfo | undefined {
  ...
  if (props.newTabData.brandedWallpaper && props.newTabData.brandedWallpaper.type === 'richMedia') {
    const wallpaperData = props.newTabData.brandedWallpaper
    if (wallpaperData.wallpaperImageUrl) {
      return {
...
class NewTabPage extends React.Component<Props, State> {
  ...
  componentDidUpdate (prevProps: Props) {
    ...
    if (newSponsoredRichMediaBackground && oldSponsoredRichMediaBackground !== newSponsoredRichMediaBackground) {
      this.trackCachedRichMediaBackground()
    }

    if ((oldImageSource &&
      !newImageSource) || (oldSponsoredRichMediaBackground && !newSponsoredRichMediaBackground)) {
      // reset loaded state
  ...
  trackCachedRichMediaBackground () {
    ...
    if (this.sponsoredRichMediaBackgroundInfo) {
      ...
      richMediaBackgroundCache.src = this.sponsoredRichMediaBackgroundInfo.url
  ...
  render () {
    ...
    const hasSponsoredRichMediaBackground = this.sponsoredRichMediaBackgroundInfo !== undefined
    ...
    return (
      <Page.App
        ...
        sponsoredRichMediaBackgroundHasLoaded={this.state.backgroundHasLoaded}
...

        {
          ...
          <SponsoredRichMediaBackground
            ...
            sponsoredRichMediaBackgroundHasLoaded={this.state.backgroundHasLoaded}
            onEventReported={(eventType: NewTab.SponsoredRichMediaEventType) => {
              triggerSponsoredRichMediaEvent(this.props.newTabData.brandedWallpaper, eventType)

              if (this.sponsoredRichMediaBackgroundInfo && eventType === 'click') {
